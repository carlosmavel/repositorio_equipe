...
def test_os_detalhar_exibe_respostas_formulario(client):
    login_admin(client)
    with app.app_context():
        proc = Processo(nome='ProcR')
        etapa = ProcessoEtapa(nome='EtapaR', ordem=1, processo=proc)
        cel = Celula.query.first()
        admin_user = User.query.filter_by(username='admin').first()
        form = Formulario(
            nome='FormR',
        ...
        respostas = FormularioResposta(dados={"1": "Resposta"})
