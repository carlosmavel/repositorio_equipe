...
def create_diagram(user, title, document=None, *, celula_id=None, assets=(), session=None):
...
def copy_template(user, template, *, title=None, session=None):
    ...
    return create_diagram(user, title or f'Cópia de {template.title}', template.document,
                          celula_id=user.celula_id, session=session)
